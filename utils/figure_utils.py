...
def sexes_gene_expression_comparison(ad_data, gene_name, herm_color='blue', male_color='red', custom_form={'interline': 0.07, 'text_height': 1.07, 'color': 'gray', 'line_placement': -0.1}, filename=None):
    ...
    fig.update_layout(
        ...
        showlegend=True,
        title=dict(text=f'Hermaphrodite & Male <i><b>{gene_name}</b></i><br>Expression', font=dict(size=25)),
        font=dict(family='Rockwell'),
        margin=dict(pad=20),
    )

    fig.update_xaxes(showgrid=False, tickangle=90, tickfont=dict(family='Rockwell', color='gray', size=18),
                     showline=False, gridcolor='lightgray')
    fig.update_yaxes(showgrid=False, tickfont=dict(family='Rockwell', color='gray', size=20),
                     showline=False, gridcolor='lightgray', range=[-0.001, max_val + 0.5])
